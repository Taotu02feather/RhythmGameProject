...
int main(int,char**){
    ...
    engine.GetGameLoop()->SetOnRender([&](double){Ore::Renderer*r=rdr;int W=cfg.windowWidth,H=cfg.windowHeight,CX=W/2;
        ...
        switch(cur){
        case Page::MainMenu:r->ClearScreen(14,14,30);r->DrawRect(0,0,W,75,8,8,22);DT(r,"Open Rhythm Engine",20,10,255,220,80,255);DT(r,"v0.2.0",20,35,180,180,200,255);
            {const char*ls[3]={"开始游戏","设置","退出"};int cs[3][3]={{40,140,220},{80,80,180},{200,60,60}};
            for(int i=0;i<3;++i){int y=150+i*90,bx=CX-160;if(ms==i){r->DrawRect(bx-3,y-3,326,56,255,200,60,200);r->DrawRect(bx,y,320,50,cs[i][0]+40,cs[i][1]+30,cs[i][2]+20,255);}else r->DrawRect(bx,y,320,50,cs[i][0],cs[i][1],cs[i][2],200);
            int tw=r->GetPixelTextWidth(ls[i],3);DT(r,ls[i],CX-tw/2,y+10,255,255,255,255);}}DT(r,"UP/DOWN:选择  ENTER:确认  ESC:退出",CX-150,600,160,160,200,255);break;
        ...
        case Page::Play:r->ClearScreen(14,14,26);if(rdy){r->ClearScreen(14,14,28);DT(r,"谱面完成!",CX-60,200,255,255,100,255);snprintf(b,sizeof(b),"总分: %d",sc.ts);DT(r,b,CX-80,260,255,255,255,255);DT(r,"按 ENTER 查看详情",CX-80,320,200,200,220,255);break;}
            ...
            if(el-laneF[i]<0.8){
                ...
                r->DrawPixelText(lbuf,x+2,ly-18,lr,lg,lb,255,1);}}
